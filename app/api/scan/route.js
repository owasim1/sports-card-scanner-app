...
export async function POST(req) {
  ...
  // const imageUrl = "https://i.ebayimg.com/images/g/1bcAAOSw5ftlgUHl/s-l400.jpg";
  if (!imageUrl)
    return NextResponse.json({ error: "Image URL required" }, { status: 400 });

  console.log("📸 Scanning:", imageUrl);
  try {
    console.log("📸 Scanning with GPT-4 Vision:", imageUrl);
      ...
      messages: [
        {
          ...
          content: [
            {
              type: "text",
              text: `You are analyzing a sports trading card image. Please respond in JSON format with the following fields:
              {
              "player": string,
              "team": string,
              "brand": string,
              ...
              "features": string[] // such as "rookie", "autograph", "holo", "graded", etc
              }
    ...
    const clean = gptResult
      .replace(/```json/i, "") // Remove ```json
      .replace(/```/g, "") // Remove ```
      .trim();

    const cardInfo = JSON.parse(clean);
    console.log("🧠 GPT Vision result:", cardInfo);
    // const cardInfo = {
    ...
    //   year: 1991,
    //   card_number: "C",
    //   card_full_name: "1991 Score Mario Lemieux",
    //   features: ["autograph"],
    // };
    // Optional: Try to query SportsCardsPro using the GPT result (you can tweak this logic)
    const sportscardsproResponse = await axios.get(
      `https://www.sportscardspro.com/api/products?t=${SPORTSCARDSPRO_API_KEY}&q=${encodeURIComponent(cardInfo.card_full_name)}`,
    );
    ...
    await sql`
    INSERT INTO scans (ip, gpt_result, product_data, image_url)
    VALUES (
      ${clientIp},
      ${cardInfo},
